  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      try {
        ...
        // If we get a permission error, it usually means the user doesn't exist 
        // and rules are restricting read access to existing users only.
        console.error('Firestore Read Error:', err);
        await auth.signOut();
        toast.error('Access denied. Please ensure you have created an account first.');
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#f5f5ee] dark:bg-[#0a0a0a] transition-colors duration-500">
      <div className="glass w-full max-w-md p-8 animate-fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-sm opacity-60">Connecting batchmates, building futures.</p>
        </div>
